    ...
    override fun getGlobalButtons(player: Player) = mapOf(
        4 to ItemBuilder
            ...
            .toButton { _, _ ->
                ...
                stage2PartyPlay(player, team1, team2).openMenu(player)
            }
    )

    private fun stage2PartyPlay(player: Player, team1: List<UUID>, team2: List<UUID>) = object : TemplateKitMenu(player)
    {
        ...
        )
        {
            val gameExpectation = GameExpectation(
                UUID.randomUUID(),
                players = (team1 + team2).toMutableSet(),
                teams = setOf(
                    GameTeam(TeamIdentifier.A, team1.toMutableSet()),
                    GameTeam(TeamIdentifier.B, team2.toMutableSet())
                ),
                kitId = kit.id,
                mapId = map.name,
                queueId = "party"
            ...
            lobbyPlayer.partyOf().onlinePlayers()
                .let {
                    if (!gameExpectation.players.all { player -> player in it })
                    {
