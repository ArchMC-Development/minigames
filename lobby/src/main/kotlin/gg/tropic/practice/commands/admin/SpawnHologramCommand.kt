...
{
    @PreCommand
    fun onPreCommand(sender: ScalaPlayer)
    {
        if (MinigameLobby.isMinigameLobby() || MinigameLobby.isMainLobby())
        {
            throw ConditionFailedException("This command is unavailable on Minigame/Main lobbies, please use /manageminigamelobby instead.")
        }
    }
    ...
    fun customize(manager: ScalaCommandManager)
    {
        manager.commandCompletions.registerStaticCompletion(
            "leaderboard-types",
            CommonlyViewedLeaderboardType.entries
                .map(CommonlyViewedLeaderboardType::name)
        )
    }
    ...
    )
    {
        val hologram = ScrollingTypeLeaderboardHologram(
            leaderboardTypes = leaderboardTypes
                .split(",")
                .map(CommonlyViewedLeaderboardType::valueOf),
            scrollTime = delay,
    ...
    fun onScrollingKits(
        player: ScalaPlayer, delay: Int,
        @Single kits: String,
        leaderboardType: CommonlyViewedLeaderboardType
    )
    {
