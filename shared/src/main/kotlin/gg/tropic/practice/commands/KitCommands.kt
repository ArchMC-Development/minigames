...
object KitCommands : ScalaCommand()
{
    ...
    fun onClone(player: ScalaPlayer, kit: Kit, id: String)
    {
        ...
        with(KitService.cached()) {
            KitService.cached().kits[newKit.id] = newKit
            KitService.sync(this)
        }

        player.sendMessage(
            "${CC.GREEN}You have created a new cloned kit $id."
        )
    }

    @AssignPermission
    @Subcommand("number-of-teams")
    @CommandCompletion("@kits")
    @Description("Change the game lifecycle of a kit.")
    fun onNumberTeams(player: ScalaPlayer, kit: Kit, teams: Int)
    {
        if (kit.lifecycle() != GameLifecycle.MiniGame)
        {
            throw ConditionFailedException("You can only set the number of teams when the lifecycle is MiniGame!")
        }

        if (teams < 2 || teams > 10)
        {
            throw ConditionFailedException("You can only have a maximum of 10 teams and must have a minimum of 2 teams!")
        }

        val oldNumberTeams = kit.numberOfTeams()
        kit.numberOfTeams = teams
        with(KitService.cached()) {
            KitService.cached().kits[kit.id] = kit
            KitService.sync(this)
        }
        ...
        player.sendMessage(
            "${CC.GREEN}You have updated the # of teams of the ${CC.YELLOW}${kit.displayName}${CC.GREEN} kit from ${CC.PRI}$oldNumberTeams${CC.GREEN} to ${CC.PRI}$teams${CC.GREEN}."
        )
    }
    ...
    @CommandCompletion("@kits")
    @Description("Change the game lifecycle of a kit.")
    fun onGameLifecycle(player: ScalaPlayer, kit: Kit, newLifecycle: GameLifecycle)
    {
        val oldLifecycle = kit.lifecycle()
        kit.lifecycleType = newLifecycle
        if (newLifecycle != GameLifecycle.MiniGame)
        {
            kit.numberOfTeams = 2
            player.sendMessage(
                "${CC.GREEN}Updating number of teams: 2."
            )
        }

        val removedFlags = mutableSetOf<FeatureFlag>()
        for (key in kit.features.keys.toSet())
        {
            if (newLifecycle !in key.availableLifecycles)
            {
                kit.features.remove(key)
                removedFlags += key
            }
        }
        ...
        player.sendMessage(
            "${CC.GREEN}You have updated the lifecycle type of the ${CC.YELLOW}${kit.displayName}${CC.GREEN} kit from ${CC.PRI}$oldLifecycle${CC.GREEN} to ${CC.PRI}$newLifecycle${CC.GREEN}."
        )

        if (removedFlags.isNotEmpty())
        {
            player.sendMessage(" ${CC.GRAY}As a result, ${removedFlags.size} incompatible feature flags were removed.")
            for (removedFlag in removedFlags)
            {
                player.sendMessage("  - $removedFlag")
            }
        }
    }
    ...
    @Subcommand("tools golden-head")
    @Description("Get a golden head.")
    fun onGoldenHead(player: ScalaPlayer)
    {
        player.bukkit().inventory.addItem(
            ItemBuilder
                .of(Material.GOLDEN_APPLE)
                .name("${CC.GOLD}Golden Head ${CC.GRAY}(Right Click)")
                .build()
        )
    }

    @Subcommand("tools fireball")
    @Description("Get a fireball.")
    fun onFireball(player: ScalaPlayer)
    {
        player.bukkit().inventory.addItem(
            ItemBuilder
                .of(Material.FIREBALL)
                .name("${CC.GOLD}Fireball ${CC.GRAY}(Right Click)")
                .build()
        )
    }
