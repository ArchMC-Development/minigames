 ...
 * @since 11/10/2022
 */
data class MapEnderChestMetadata(
...
) : AbstractMapMetadata()
{
    ...
    override fun report() = "EChests | $id | ${locations.size} group (${locations.entries.joinToString(", ") { "${it.key} -> ${it.value.size}"}})"
    override fun getAbstractType() = MapEnderChestMetadata::class.java
}
