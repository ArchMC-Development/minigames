 ...
 * @since 11/10/2022
 */
data class MapTeamUpgraderMetadata(
...
) : AbstractMapMetadata()
{
    ...
    override fun report() = "TeamUpgrader | $id | ${locations.size} group (${locations.entries.joinToString(", ") { "${it.key} -> ${it.value.size}"}})"
    override fun getAbstractType() = MapTeamUpgraderMetadata::class.java
}
