...
class PlayerViewMenu(
    ...
    if (snapshot.playerUniqueId in Globals.POSSIBLE_PLAYER_BOT_UNIQUE_IDS) "Robot" else gameReport.usernameOf(snapshot.playerUniqueId)
}...")
{
    ...
    override fun getButtons(player: Player): Map<Int, Button>
    {
        ...
        buttons[48] = ItemBuilder.of(XMaterial.COOKED_BEEF)
        ...
        if (snapshot != null)
        {
            buttons[53] = ItemBuilder
                ...
                .name(
                    "${CC.B_SEC}${
                        if (nextPlayer in Globals.POSSIBLE_PLAYER_BOT_UNIQUE_IDS) "Robot" else gameReport.usernameOf(nextPlayer)
                    }'s Inventory"
                )
                .addToLore(
                    "",
