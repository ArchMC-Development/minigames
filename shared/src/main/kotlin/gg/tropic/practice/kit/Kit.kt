...
)
{
    ...
    @Transient
    private var backingAllowedBlockTypeMappings: Optional<Map<Material, Int>>? = null

    val allowedBlockTypeMappings: Optional<Map<Material, Int>>
        get()
        {
    ...
    var lifecycleType: GameLifecycle? = GameLifecycle.SoulBound
    var numberOfTeams: Int? = 2

    fun numberOfTeams() = numberOfTeams ?: 2
    fun lifecycle() = lifecycleType ?: GameLifecycle.SoulBound
    ...
    override fun equals(other: Any?): Boolean
    {
        ...
        if (backingAllowedBlockTypeMappings != other.backingAllowedBlockTypeMappings) return false
        if (backingQueueSizes != other.backingQueueSizes) return false
    ...
    override fun hashCode(): Int
    {
        ...
        result = 31 * result + (backingAllowedBlockTypeMappings?.hashCode() ?: 0)
        result = 31 * result + (backingQueueSizes?.hashCode() ?: 0)
        return result
    }
}
